...
export default function RegisterPage() {
  ...
  // Show confirm-password mismatch error 3 s after the user stops typing,
  // but only when both fields are non-empty and actually differ.
  const matchTimerRef = useRef(null);
  useEffect(() => {
    clearTimeout(matchTimerRef.current);

    // If either field is empty, or they already match, clear any existing error.
    if (
      !formData.confirmPassword ||
      formData.password === formData.confirmPassword
    ) {
      setErrors((prev) =>
        prev.confirmPassword ? { ...prev, confirmPassword: null } : prev,
      );
      return;
    }
  ...
  const validate = () => {
    ...
    if (selectedRole === "student") {
      const regResult = validateRequired(
    ...
    if (selectedRole === "store_employee") {
      ...
      if (!upiResult.valid) {
        ...
        if (!upiFormatResult.valid)
          newErrors.storeUpiId = upiFormatResult.error;
      }
    }
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {(selectedRole === "student" ||
                      ...
                      <p className="text-xs text-muted-foreground">
                        Use your official Christ University email. Subdomains
                        like
                        <span className="font-medium">
                          {" "}
                          course.christuniversity.in
                        </span>{" "}
                        are accepted.
                      </p>
                    )}
