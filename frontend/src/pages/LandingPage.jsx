    ...
    <div className="relative min-h-screen overflow-hidden bg-transparent text-foreground">
        ...
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Campus food ordering</p>
            </div>
          </div>
        ...
        <div className="mt-10 grid items-center gap-10 lg:mt-14 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <h1 className="max-w-xl text-4xl leading-tight tracking-tight sm:text-5xl lg:text-6xl">
              Skip the queue.
              <span className="block text-orange-700">Pick up food when it is ready.</span>
            </h1>

            <p className="mt-4 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">
              CampusBite helps students, faculty, and store teams manage food orders faster with
              clean menus, instant checkout, and real-time status updates.
            </p>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <Button size="lg" variant="outline" className="justify-center" asChild>
            ...
            <div className="mt-8 grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3">
              {STATS.map((stat) => (
                <div key={stat.label} className="rounded-xl border border-border/80 bg-white/80 px-4 py-3 shadow-sm">
          ...
          <Card className="border-amber-200/80 bg-white/90 shadow-xl">
            <CardContent className="space-y-5 p-5 sm:p-6">
              <p className="text-sm font-semibold text-foreground">How CampusBite helps daily</p>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-border/80 bg-white/85 p-5 shadow-lg sm:p-8">
          <div className="mb-6 flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-muted-foreground">Order flow</p>
          </div>

          <div className="grid gap-4 md:grid-cols-3">
            {STEPS.map((step) => (
              <div key={step.number} className="rounded-2xl border border-border/70 bg-background/70 p-4">
                <p className="text-xs font-semibold tracking-[0.15em] text-orange-700">{step.number}</p>
      ...
      <footer className="border-t border-border/80 bg-white/70">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-center px-4 py-6 text-sm sm:px-6 lg:px-8">
          <p className="text-muted-foreground">&copy; {new Date().getFullYear()} CampusBite</p>
        </div>
      </footer>
    </div>
  )
