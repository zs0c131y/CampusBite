...
const ORDER_NOTIFICATION_MAP = {
  accepted: {
    ...
    message:
      "Your payment was confirmed and the store has accepted your order.",
  },
  processing: {
    title: "Order Being Prepared 👨‍🍳",
    message: "The store is now preparing your order. Hang tight!",
  },
  ready: {
    title: "Order Ready for Pickup 🔔",
    message:
      "Your order is ready! Head to the store and show the OTP to collect it.",
  },
  picked_up: {
    title: "Order Collected 🍽️",
...
const pushOrderNotification = (userId, order, status, extra = {}) => {
  const meta = ORDER_NOTIFICATION_MAP[status] || {
    title: "Order Update",
...
const UNPAID_ORDER_TIMEOUT_MINUTES = Number(
  process.env.UNPAID_ORDER_TIMEOUT_MINUTES || 8,
);
const READY_NO_SHOW_TIMEOUT_MINUTES = Number(
  process.env.READY_NO_SHOW_TIMEOUT_MINUTES || 20,
);
const ORDER_COMMITMENT_TIMEOUT_MINUTES = Number(
  process.env.ORDER_COMMITMENT_TIMEOUT_MINUTES || 4,
);
const NO_SHOW_WARNING_THRESHOLD = Number(
  process.env.NO_SHOW_WARNING_THRESHOLD || 2,
);
const NO_SHOW_RESTRICTION_THRESHOLD = Number(
  process.env.NO_SHOW_RESTRICTION_THRESHOLD || 3,
);
const NO_SHOW_RESTRICTION_DAYS = Number(
  process.env.NO_SHOW_RESTRICTION_DAYS || 14,
);
...
const applyNoShowPenalty = async (userId) => {
  ...
  if (user.no_show_count >= NO_SHOW_RESTRICTION_THRESHOLD) {
    user.ordering_restricted_until = nowPlusMinutes(
      NO_SHOW_RESTRICTION_DAYS * 24 * 60,
    );
  ...
  for (const item of normalizedItems) {
    ...
    if (menuItem.store_id.toString() !== storeId.toString()) {
      const error = new Error(
        `Item "${menuItem.name}" does not belong to this store.`,
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    res.json({
      ...
      data: {
        ...
        store: {
          ...
          qrCodeUrl: draft.store.qr_code_url || null,
        },
        ...
        payment: {
          ...
          storeQrCodeUrl: draft.store.qr_code_url || null,
          paymentReference,
        },
      },
    });
...
export const createOrder = async (req, res, next) => {
  try {
    ...
    if (
      Math.abs(draft.totalAmount - Number(checkoutData.totalAmount || 0)) > 0.01
    ) {
      return res.status(409).json({
        success: false,
        message:
...
export const getOrderById = async (req, res, next) => {
  try {
    ...
    const order = await Order.findById(id)
      ...
      .populate(
        "user_id",
        "name email phone_number role register_number employee_id",
      );
    ...
    if (role === "store_employee") {
      ...
      if (
        !ownerStore ||
        ownerStore._id.toString() !== order.store_id._id.toString()
      ) {
        return res.status(403).json({
          success: false,
