...
const isChristUniversityEmail = (email) => {
  ...
  return (
    domain === CHRIST_UNIVERSITY_DOMAIN ||
    domain.endsWith(`.${CHRIST_UNIVERSITY_DOMAIN}`)
  );
};
...
export const registerSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      data.role === "student" &&
      (!data.registerNumber || data.registerNumber.trim() === "")
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (
      (data.role === "faculty" || data.role === "store_employee") &&
...
export const register = async (req, res, next) => {
  try {
    ...
    if (
      ...
      !isChristUniversityEmail(normalizedEmail)
    ) {
...
export const verifyEmail = async (req, res, next) => {
  try {
    ...
    // Token found — if already verified, just return success (idempotent).
    if (user.is_email_verified) {
      return res.json({
        success: true,
    ...
    // Keep the token on the record so the same link stays idempotently valid.
    await user.save();
